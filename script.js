...
function updateStatus(action, status) {
    ...
    const statusMessage = `
       Medicine Batch Block Created: ${actionStatus.create}<br><br>
       Transfer Batch Block Created: ${actionStatus.transfer}<br><br>
       Delivery Batch Block Created: ${actionStatus.delivery}
    `;
    showPopup("Transaction Status Updated", "All Actions Status", statusMessage);
}
...
async function createMedicine() {
    ...
    showSpinner(); // Show spinner when the button is clicked

    try {
        await contract.methods.createMedicine(medicineName, batchNumber).send({ from: accounts[0] });
        actionStatus.create = "Successfully";
        updateStatus("create", "Successfully");
    } catch (error) {
        actionStatus.create = "Failed";
        updateStatus("create", `Failed: ${error.message}`);
    } finally {
        hideSpinner(); // Hide spinner after the transaction
    }
}
...
async function transferMedicine() {
    ...
    try {
        ...
        actionStatus.transfer = "Successfully";
        updateStatus("transfer", "Successfully");
    } catch (error) {
...
async function confirmDelivery() {
    ...
    try {
        ...
        actionStatus.delivery = "Successfully";
        updateStatus("delivery", "Successfully");
    } catch (error) {
...
async function showDetails() {
    ...
    showSpinner(); // Show spinner during fetching data
